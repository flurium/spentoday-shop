...
<div class="mt-12 mb-6">
  <div class="flex flex-col-2 gap-6 mb-6">
    <div 
     class="relative ms-12 me-6 basis-1/2">
  <div
  ...
  <button
    ...
    <h1 class="font-bold text-3xl text-secondary-700">{data.single.product.name}</h1>
    <div class="text-xl text-gray-600 m-y-12">₴{data.single.product.price}грн.</div>
    <div class="text-xl text-gray-600 m-y-12">{data.single.product.description}</div>
      <div class="flex flex-col-2">
          {#if $cart.some((x) => x.id == id)}
          <button class="px-5 rounded-full border me-4 h-2/5 place-self-center border-gray-400" on:click={() => cart.remove(id)}>Видалити з кошику</button>
          {:else}
          <button
            on:click={() =>
              cart.add({
                id: id,
                name: data.single.product.name,
                price: data.single.product.price,
                amount: amount
              })}
...
<div class="ms-10 mt-10 me-10">
  <div class="flex flex-col-3 w-full gap-x-10 border-b border-gray-400">
    <button class="border-b px-4 font-bold  border-secondary-700 text-secondary-700 bottom-0">Опис</button>
    <button class="px-4 font-bold text-secondary-500">Додаткова інформація</button>
    <button class="px-4 font-bold text-secondary-500">Відгуки</button>
  </div>
  <div>{data.single.product.seoDescription}</div>
</div>

  <div class="grid grid-col-4 ms-10">
    <div class="max-w-2xl py-16 sm:py-24 lg:max-w-7xl">
      <h2 class="text-4xl font-bold text-gray-900">ТАКОЖ МОЖЕ <br /> СПОДОБАТИСЬ</h2>
